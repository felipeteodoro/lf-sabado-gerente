...
Você é um agent de coding (Hermes Agent, no caso) lendo este arquivo via convenção `AGENTS.md` na raiz. Este arquivo é auto-aplicado a cada sessão do Hermes — leia antes de propor mudanças.

> **Convenção mantida**: o conteúdo deste arquivo é **idêntico** a `docs/agents-guide.md` e `.github/copilot-instructions.md`. Atualize os três juntos se algo mudar.
...
Este projeto mantém o conteúdo de orientação de AI em três endereços, **idênticos**:
- `AGENTS.md` (raiz) — convenção do Hermes Agent, auto-aplicado a cada sessão
- `docs/agents-guide.md` — entrada única legível por qualquer agent
- `.github/copilot-instructions.md` — auto-aplicado pelo GitHub Copilot em code review

Se você atualizar um, atualize os outros.
